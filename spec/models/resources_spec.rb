...
describe Resources do
  describe '.current' do
    ...
    context 'with a custom setting value' do
      before do
        setting = instance_double(Setting, value: 'Resources', updated_at: 10.days.ago)
        allow(Setting).to receive(:find_by).with(var: 'site_resources').and_return(setting)
      end

      it 'has the privacy text' do
        record = described_class.current

        expect(record.text).to eq('Resources')
      end
    end
  end
end
